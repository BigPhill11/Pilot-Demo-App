...
import { EMAIL_LESSON_STEP_LABELS } from '@/data/career-readiness/email-etiquette';
import {
  INTERVIEW_STEP_ORDER,
  canReviewEmailLessonStep,
  emailLessonStepKey,
...
interface EmailStepNavigatorProps {
...
const EmailStepNavigator: React.FC<EmailStepNavigatorProps> = ({
  lessonId,
  details,
  currentStepId,
  onNavigate,
  ...
  return (
    <div className="grid grid-cols-4 gap-1">
      {INTERVIEW_STEP_ORDER.map((stepId, index) => {
        const labels = EMAIL_LESSON_STEP_LABELS[stepId];
        ...
        const canOpen = canReviewEmailLessonStep(details, lessonId, stepId);
...
export default EmailStepNavigator;
