...
interface InterviewLessonStepperProps {
  details: InterviewModuleDetails;
  currentLessonId: InterviewLessonId;
  onSelectLesson: (lessonId: InterviewLessonId) => void;
}

const InterviewLessonStepper: React.FC<InterviewLessonStepperProps> = ({
  details,
  currentLessonId,
  onSelectLesson,
}) => {
  return (
    <div className="flex gap-2 overflow-x-auto pb-1">
      {INTERVIEW_LESSON_ORDER.map((lessonId, index) => {
        const config = INTERVIEW_LESSONS.find((l) => l.id === lessonId)!;
        const complete = isLessonComplete(lessonId, details.completedLessonSteps);
        const active = currentLessonId === lessonId;
        const canOpen = canNavigateToLesson(details, lessonId);
...
export default InterviewLessonStepper;
