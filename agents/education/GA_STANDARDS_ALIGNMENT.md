...
|---|---|---|
| **Taxes** (`personal-finance/taxes/`, boss: *The Tax Strategist*) | SSEPF5a (tax types) — **PARTIAL** (verified 2026-07-06, downgraded from "probable-STRONG"): income tax + payroll (Social Security/Medicare) taught strongly (`lesson-1`, `lesson-2`), capital gains taught (`lesson-2` flashcard); **sales, property, and estate taxes only named in passing** in one `lesson-1` flashcard definition, never taught (confirmed final 2026-09-20 — lessons 3-5 and the boss game don't touch them either). SSEPF5b (progressive/regressive/proportional) — **PARTIAL, upgraded 2026-09-21**: `lesson-5-tax-planning-mindset.ts` now teaches the progressive-bracket mechanic directly (a new microLesson paragraph, a "Tax Bracket" flashcard with a stacked-cups analogy, and a decision-style quiz item on how a raise affects take-home pay), closing the "bracket is never taught" gap confirmed 2026-09-20. Still not covered: regressive and proportional structures, and no other lesson in the module uses the words progressive or marginal. Sales, property, and estate taxes are also never taught in lessons 3-5 or the boss game (confirming SSEPF5a completion is still the item below). SSEPF2b (complete a 1040) — **GAP** (no form-completion activity evidenced); SSEPF3d retirement accounts — PARTIAL (`lesson-4-tax-advantaged-accounts.ts`). **Bonus:** `lesson-2` also strongly reinforces SSEPF2a (income types: wages, self-employment, investment, dividends, capital gains). |
...
| **Wealth Fundamentals** (`personal-finance/wealth-fundamentals/`, boss: *The Wealth Builder*) | SSEPF2f (net worth = assets − liabilities) — **STRONG** (verified 2026-07-06, upgraded from "probable"): taught in `lesson-1` microLesson, Asset/Liability flashcards, quiz item 1, and the "track your net worth monthly" powerMove. SSEPF1e (generational wealth) — **PARTIAL** (verified 2026-07-06, downgraded from "probable-STRONG"): `lesson-1` teaches asset ownership vs. income and long-term stability, but has **no explicit generational-transfer / inheritance / passing-wealth-forward content** — the element's core idea is absent. **Confirmed final 2026-09-20:** read `lesson-3-risk-life-stages.ts` in full — it's a strong, well-built lesson on how risk tolerance should shift across life stages (bamboo-forest-season analogies, a 3-stage simulator), but it's about *investment risk over time*, not wealth transfer — no legacy, inheritance, estate, or passing-wealth-forward content anywhere in the module. SSEPF1e generational-wealth content is genuinely absent app-wide; this is a targeted content add, not a place needing more reading. SSEPF4d reinforced (`lesson-2-systems-compounding.ts`). |
...
11. **SSEPF5b — progressive / regressive / proportional tax structures** (upgraded PARTIAL 2026-09-21 — see §3; bracket/marginal-rate mechanic now taught in `lesson-5`). Still missing: regressive (e.g. sales tax hits lower incomes harder) and proportional (flat-rate) structures, and a "who pays what share" comparison visual — good candidate for the next pass.
...
**Cleared 2026-09-20:**
6. ✅ `wealth-fundamentals/lesson-3-risk-life-stages.ts` — read in full. SSEPF1e generational-wealth GAP **confirmed final** (lesson covers risk-across-life-stages, not wealth transfer; no legacy content anywhere in the module).
7. ✅ `taxes/lesson-3`, `lesson-4`, `lesson-5`, and boss game *The Tax Strategist* — read in full (previous pass only grepped). SSEPF5b progressivity GAP **confirmed final** ("bracket" used as flavor text twice, never taught); SSEPF5a sales/property/estate GAP **confirmed final** (never mentioned in lessons 3-5 or the boss game).

**Still open:**
8. **Surface this mapping in `src/data/teacher-curriculum-guide.md`** — DONE 2026-07-06 (see the "Georgia standards alignment (SSEPF)" section added to the guide; copy addition, auto-apply eligible).
9. **Per-lesson `gaStandards: ['SSEPF6b', ...]` tags** — **PROPOSE-ONLY / BLOCKED.** Verified 2026-07-06 that `src/types/personal-finance.ts` `Lesson` has no `gaStandards` field and no index signature, so adding tags to lesson objects would fail the type-check (excess-property error). Applying tags therefore requires first adding an optional `gaStandards?: string[]` to the `Lesson` (and `PersonalFinanceModule`) interface — a `src/types/` change, which is high-risk under the 2026-07-05 policy. See WORDING_ENGAGEMENT_LOG proposal **STD-TAG-1** (awaiting Phil's APPROVED).
10. **All six queued verification items are now cleared.** Next open task per the 2026-07-06 sweep queue: draft the Week-1 (Income) pre/post assessment item blueprint in `ASSESSMENT_SPEC.md` v0.2 (doc-only, auto-apply).
