...
| **Wealth Fundamentals** (`personal-finance/wealth-fundamentals/`, boss: *The Wealth Builder*) | SSEPF2f (net worth = assets − liabilities) — **STRONG** (verified 2026-07-06, upgraded from "probable"): taught in `lesson-1` microLesson, Asset/Liability flashcards, quiz item 1, and the "track your net worth monthly" powerMove. SSEPF1e (generational wealth) — **STRONG** (updated 2026-09-25, upgraded from "PARTIAL"): `lesson-1` now teaches generational wealth directly — a new microLesson paragraph, a "Generational Wealth" flashcard (bamboo-grove-inherited-already-growing analogy), and a quiz item distinguishing it from high current income. `lesson-3-risk-life-stages.ts` is confirmed to cover investment-risk-by-age only, not wealth transfer, so the new content in `lesson-1` is what closes this element. SSEPF4d reinforced (`lesson-2-systems-compounding.ts`). |
...
8. ~~Verify `wealth-fundamentals/lesson-3-risk-life-stages.ts`~~ — **DONE 2026-09-25**: confirmed investment-risk-by-age only, no legacy/transfer content. SSEPF1e content gap closed instead via a `lesson-1` addition (see §3 above).
